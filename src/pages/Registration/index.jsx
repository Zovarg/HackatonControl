...
function Registration() {
    ...
    const [checkRegl, setCheckRegl] = useState(false);
    const [fetchRegistration, isPostsLoading, postError] = useFetching(async (info) => {
        await api.auth.registration(info);
        let obj={email:info.email, password:info.password}
    ...
    function changeRegl(){
        setCheckRegl(!checkRegl)
    };
    function authoriz(e){
        e.preventDefault();
        nav("/");
    };
    setTimeout(() => { Inputmask('+9(999)999-99-99').mask(document.getElementsByClassName("phone-input")) }, 1)
  return (
              ...
              <div className={cl.container__body}>
                  <div className={cl.container__body_title}>
                      <div className={cl.bodyTitleWrapper}>
                          <div className={cl.bodyTitleInfo}>Регистрация</div>
                  ...
                  <div className={cl.formAndBtns}>
                      <div className={cl.container__body_form}>
                          ...
                          <div className={cl.formInputThird}>
                              <input
                                  type="password"
                                  placeholder="Пароль"
                          ...
                          <div className={cl.formCheck}>
                              <div className={cl.formCheck__infoPas}>Используйте более 8 разных символов, букв и цифр </div>
                              <div className={cl.formCheck__password}>
                                  <div className={cl.formInputFour}>
                                      <input
                                          type="password"
                                          placeholder="Повторите пароль"
                                          value={checkPas}
                                          onChange={e=>setCheckPas(e.target.value)}
                                      />
                                  </div>
                              </div>
                              <div className={cl.formCheck__reglament}>
                                  <div className={cl.formCheck__reglament_checkbox}>
                                      <input
                                          id="reglamentCheck"
                                          type="checkbox"
                                          value={checkRegl}
                                          onChange={()=>setCheckRegl(!checkRegl)}
                                      />
                                  </div>
                                  <div className={cl.formCheck__reglament_info}><label htmlFor="reglamentCheck">Я принимаю <span>условия</span> и политику <span>конфиденц.</span></label></div>
                              </div>
                          </div>
                      </div>
                      <div className={cl.container__body_btns}>
                          <div className={cl.btnSubmit} onClick={submitForm}>Регистрация</div>
                          <div className={cl.btnRedirect}>
                              <div className={cl.btnRedirect__info}>Уже есть аккаунт?</div>
                              <div className={cl.btnRedirect__ref} onClick={authoriz}>Войти</div>
                          </div>
                      </div>
                  </div>
              </div>
